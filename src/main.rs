...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    match opts.subcmd {
        SubCommandKind::Download => download(slack_client, &opts.target_directory).await,
        SubCommandKind::Upload => upload(slack_client, &opts.target_directory).await,
    }
}
